...
**must propagate to all the parts of the body before the body can respond to it in any way**. Why?
...
a rigid body to respond to a collision event. Note that this latency adds to the latency
we discussed in the previous section stemming from the Born rigidity constraint.
